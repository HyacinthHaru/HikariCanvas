...
**实时投影链路：**
- 玩家游戏内 `/hc edit` 锁定墙面 → 生成一次性 token + URL
...
**命令辅助：**
- `/hc edit` — 锁定墙面、开启编辑会话、返回 URL
- `/hc cancel` — 取消会话，归还预览地图
- `/hc cleanup` — 管理员清理废弃成品地图
- `/hc undo` — 回退最近一次 commit
...
| 项 | 选择 | 说明 |
| --- | --- | --- |
| 语言 | Java 21 | Paper 1.21 要求 |
| 构建 | Gradle (Kotlin DSL)，多模块 | 插件 + 前端子模块 |
| 平台 | Paper API 1.21+ | Paper 独有性能/并发 API |
| 图形 | Java AWT / Graphics2D | JDK 内置 |
| HTTP / WS | Javalin 6 | 轻量、Kotlin/Java 通吃 |
| 数据包 | PacketEvents | 直接构造 map packet，绕开 MapRenderer |
| 配置 | SnakeYAML | 模板 + 主配置 |
...
- 建立像素级对比测试台（snapshot test），CI 里跑多组文字比对
...
- Token：玩家游戏内 `/hc edit` 生成，单次使用，15 分钟过期，绑定 UUID
...
| M1 端到端验证 | 浏览器按钮 → WS → 更新游戏内一张地图像素 | 1 周 | Demo 视频 |
| M2 会话与地图池 | token 登录、墙面锁定、预览地图池借还 | 2 周 | 核心机制跑通 |
| M3 实时投影 | 差分推送、防抖节流、多图拼接、双端一致性测试 | 2 周 | 流畅编辑体验 |
| M4 渲染引擎 | 字体、调色板 LUT、效果（描边/阴影/发光） | 2 周 | 单元测试覆盖 |
...
| **浏览器 Canvas 与 Java Graphics2D 渲染不一致** | **高** | M1 就建像素级 snapshot 测试；像素字体为主、禁抗锯齿、同一 TTF |
| **`PacketEvents` 版本升级破坏兼容** | 中 | 锁版本 + CI 集成测试 + 订阅上游 release |
| **服主缺乏独立 HTTP 端口** | 中 | 文档给 nginx/Caddy 反代方案、支持路径前缀（`/hc/` 挂载） |
...
| **Paper API 版本变动** | 中 | 锁定 1.21 LTS 分支；新大版本单独出支持版 |
