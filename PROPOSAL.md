...
### 2.1 设计哲学：工具，不是保姆（2026-05-25 固化）
...
3. **不擦屁股。** 不为服主的环境兜底。网络带宽、传输压缩比、RTT、乃至服主自己在 `paper-global.yml` 里有没有开 `network-compression-threshold`——这些是**服主的运维责任**，服务端代码既探测不到、也不该花复杂度去估算。我们花精力算一个"可能的压缩率"，结果服主把压缩关了，纯属对牛弹琴。

> 这条哲学是 0.4.9 之后、规划 0.5.0 性能 Benchmark 时确立的。它直接决定 Benchmark 的形态（见 §5.2.7）：只测服务端能控制的 CPU/内存成本，绝不碰网络。
...
1. **后台模拟，不破坏世界。** Benchmark 在 async 线程跑**真实渲染代码路径**（`CanvasCompositor.rasterize` + `toPaletteSlice`），输入是程序生成的 fake `ProjectState`（招牌 / 渐变 / dither / 混合等 scene）。**不占地图池、不放 ItemFrame、不需要真实玩家 viewer、不污染 `idcounts.dat`**——可重复、随时跑、跑炸也不影响在线玩家。不学 3DMark TimeSpy 真实跑场景：HikariCanvas 渲染管线是纯软件 idempotent 函数（`ProjectState → BufferedImage`），无须真实环境触发。
...
4. **不擦屁股——网络一律不测。** 带宽、传输压缩比、RTT、丢包、服主有没有开 zlib 压缩——全部不测、不估、不算（理由见 §2.1 原则 3）。同一面墙有人拿来做 30fps 动画、有人只每秒刷一次 ETA 数值，压缩效果天差地别且取决于服主自己的配置，服务端测出来的任何"压缩率"都没有迁移意义。
...
> **进度更新（2026-06-20）**：上表为立项期（M0-M13）规划快照。M13 之后项目持续推进，已完成：M14 模板创意工坊 · M15-M16 两轮 ultrareview · M17 生产级体验组 · M18 Live Paint 油漆桶 · M19 CI+Release · M20-M26 双端字体精确化 + 20 字体矩阵 + 内置图标库 · M27 等比锁 · **0.4.0** 变量系统 + Plugin API · **0.4.1-0.4.3** chip 编辑器 + 别名 + 全局变量 · **0.4.4-0.4.5** 铁路网络 + 打磨 · **0.4.6-0.4.10** 体验打磨 + 两轮 ultrareview 修复 · **0.5.0** 纯服务端性能 Benchmark · **0.6.0** 时间轴编辑器 · **0.7.0-0.7.4** 视觉运行时（积木脚本）+ 补间动画 + 体验优化 · **0.8** 工程导入导出（.canvas）+ SVG 矢量导入 · **0.8.1** ultrareview P0-P2 修复批（26 项）。版本串当前 `0.8.1-SNAPSHOT`（0.8 在 `0.7.4-SNAPSHOT` 串下完成，2026-06-22 的 0.8.1 修复批时首次 bump）。**完整里程碑见 `CLAUDE.md` 路线图表；逐条进度以 `docs/journal.md` 为准。**

**长远 TODO（立项期登记；部分已提前在 0.4.x / 0.6 / 0.7 / 0.8 落地，下方标注）：**
- ✅ 图层缩略图（per-layer rasterize 端点）—— 0.4.8 已做
- ✅ 图层颜色标签（0.4.8）/ 图层 mask · smart object · 图层组 —— 仍属 M30 远期
- mask 完全体：✅ lasso 工具（0.4.8）/ ✅ feather 羽化（0.4.8）/ mask shape over image 拖动编辑 · 多 mask 组合 —— 仍待
- ✅ 对齐 / 分布 / 分布间距工具（Photoshop "align" 工具栏）—— M17 + 0.4.8 已做
- 多人协作 OT/CRDT（明确不做）
- 模板包生态：✅ `.canvas` 工程导入导出（0.8 单工程）—— 多模板打包 + 资产仍待
- 玩家身份认证 + HomePage 点击直开 + 权限隔离 milestone（独立路线）—— 仍待
- ✅ 动态化展示（实时数据）—— 0.4.0 变量系统 + 0.6 时间轴 + 0.7 积木脚本已覆盖；视频仍不做
