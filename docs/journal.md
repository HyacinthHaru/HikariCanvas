...
### 仓库瘦身（`2c44d93d`）
`.superpowers/`（4.7MB / 93 文件，工具本地台账）· `web/vite.config.ts.tmp.*` · `docs/template-spec 2.md` 删除；`docs/archive/template-spec.md` 移回 `docs/`（git 识别为 100% rename），`CLAUDE.md` 与 `docs/data-model.md` 的指针同步。追踪文件 890 个 / 12MB。

### 云同步污染（`9c829337`）
清理时发现工作区冒出 **361 个 `moe/hikari/canvas/**` 旧包文件**（0.9.9 改包名时删的）和 **7 个已退役 YAML 模板**（0.9.16 退的）。核实：时间戳全是 6-15 15:46、权限 `-rw-------`，与刚删掉的 `template-spec 2.md` 同签名，目录 mtime 却在删除动作之后 —— **同步服务在恢复 6 月旧快照**。同时 `plugin/build/` 里有 **46 个 ` 2.` 冲突副本**（39 个是字体）。

后果实测：`shadowJar` 被撑到 **195MB**（fonts 162.8MB + 陈旧 web 96.6MB），清理后回到 **85MB**（`web/fonts` 条目 0、无 `moe/` 类）。`moe/` 旧包是自洽完整树、**编译能过**，会作为平行包一起进 jar —— 默默发生、不报错。

`.gitignore` 补 `*.tmp.*` / `.superpowers/` / 冲突副本模式（`* 2.*` 等，确认不误伤已跟踪文件）。**但那只防提交、防不住污染构建 —— 根治需把项目目录移出同步范围。**

### cut `v0.9.17-rc.1`
0.9.17 的 202 条修复横跨渲染 / 并发 / 权限 / 协议 / 构建，测试全绿只证明「没有被测试覆盖到的回归」；地图投影、物品展示框、PacketEvents 这些与真实服务端耦合的路径单测碰不到（0.9.14 的生产三修即是先例）→ **先发 rc 交作者真服冒烟，通过后再 de-rc 到 1.0.0**。

gradle 版本串保持 `0.9.17-SNAPSHOT`，由 release.yml 的 rename 步骤产出 `HikariCanvas-0.9.17-rc.1.jar`（沿用 0.9.4 起的既定做法）—— 故游戏内 `/version HikariCanvas` 显示 `0.9.17-SNAPSHOT` 属预期。

**踩过的坑**：`9c829337` 提交信息带 `[skip ci]`，而 tag push 同属 push 事件、GitHub 照样跳过 workflow。故本条 journal 提交（不带 skip）即 tag 落点。
...
**CLAUDE.md：667 行 / 89,171 字符 → 235 行 / 22,082 字符**（每次会话常驻上下文省约 16,800 tokens）。

- **删（432 行，纯进度叙事）**：里程碑 M0→M28 叙事 / 0.4.0–0.4.6 各版本分期日志 / 0.4.x 路线图速览表 / 0.4.4 旧路线（自标「档案」）/ 0.6.0 路线 / M8–M12 / M13 路线 / M15·M16·M17·M18 实施日志（测试数、commit 哈希、jar 体积、工时估算等）。这些在 `docs/journal.md` 与 git 历史里有完整记录，写进 CLAUDE.md 只会滞后失真——文件此前自己也写着「逐条进度一律以 journal.md 为准，本表可能滞后」。
- **replaced by** 新增 `## 版本进度` 一节：只留指针（权威 = journal.md；查「什么时候做的」grep journal，查「做到哪」读 journal 顶部 + git log）。
- **保留并强化**：新增 `## 已固化的架构决策` 一节，把原先散落在各版本章节里的决策**逐条抽出按子系统归并成 42 条**（会话/鉴权/数据安全 4 · 持久化/构建 4 · 变量系统 7 · 铁路 3 · 渲染/编辑器 8 · Live Paint 5 · 模板系统 4 · 脚本/时间轴 3 · 图片蒙版 4）。原样保留：标识 / 技术栈锁定版本（含 PacketEvents GPL 约束）/ 文档先行 / Git 提交约定 / 禁 NMS 纪律 / 其他技术决策 / wall 模型 / lock 状态 / 构建速查 / CI·Release / 远期 TODO。
- **顺带修正已过时表述**：Javalin 7.1.0→7.2.2、字体矩阵 20→22 枚、「当前最新 v0.9.10-rc.1」→改为版本无关写法（不会再滞后）。
- 22 项关键契约逐条抽查全部存活（SSH 签名、禁 Claude 署名、禁 NMS、forward-only、maxPoolSize=4、IP 绑定、idcounts.dat 风险、fallback 链、params.json 契约、yaml_path 列名、charAdvance、npm ci fallback、90MB/152MB 体积真相等）。

**删除 `docs/archive/` 下 4 个 ultrareview 归档**（2026-05-16 / 05-25 / 05-29 / 06-14，共 ~2,990 行）：其修复结论早已落进代码与 journal，档案本身不再有参考价值。
