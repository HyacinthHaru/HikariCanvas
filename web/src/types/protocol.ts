...
export interface ProjectState {
    ...
    /**
     * 兼容视图：指向 activeLayer.elements 同一引用，由 project store 在 setSnapshot
     * 时建立。组件仍可读写 {@code state.elements}，新协议路径直接走 {@code layers}。
     */
    elements?: Element[];
    /** v3 新增：工程内时间轴；缺省 / null = 静态画板。 */
    timelines?: Timeline[];
    /** v3 新增：当前激活时间轴 id；缺省 / null = 无激活时间轴。 */
...
export interface Canvas {
    ...
    heightMaps: number;
    /**
     * 升级为 Fill 联合类型（solid / linear / radial），与 element fill 一致。
     * 后端 {@code FillDeserializer} 兼容旧字符串形态（自动 wrap 为 SolidFill）；
...
/**
 * 兼容形态：单 hex 字符串等价于 SolidFill。后端会一并接受，但新写出形态统一是 object。
...
export interface TextElement extends BaseElement {
    ...
    vertical: boolean;
    effects?: Effects;
    /** 加粗（内部自动转 stroke 描边模拟，避免 synthetic bold 双端差异） */
    bold?: boolean | null;
    /** 斜体（双端 shear transform 矩阵线性变换，像素一致） */
    italic?: boolean | null;
}
...
export interface IconElement extends BaseElement {
     ...
     * 图标资源名。
     *
     * - 矢量形态：{@code pack/name}（如 {@code fa-solid/heart} / {@code user/foo}）；
     *   path d + viewBox 走 {@code GET /api/icon/paths?id=...} 拉取。
     * - legacy PNG 形态：不含 {@code /}（如 {@code "heart"}）；走 {@code /api/template-asset/icons/{source}.png}。
     */
    source: string;
    /**
     * deprecated：仅供旧 .canvas / 模板向后兼容；新协议改用 {@code fill}。
     * legacy PNG 形态仍走 source-in 染色路径。
     */
    tint?: string;
    /**
     * 矢量 path 填充。Fill 联合类型（solid / linear / radial），与 Rect/Circle/Shape 共用。
...
/** legacy PNG 形态 = source 不含 `/`。镜像后端 {@code IconElement.isLegacySource}。 */
export function isLegacyIconSource(source: string | undefined | null): boolean {
    return !!source && source.indexOf('/') < 0;
}

/**
 * PathElement：通用 SVG-like 路径（M/L/Q/C/Z 子集 + marker）。
...
/** CircleElement：圆 / 椭圆，由 bbox 推 cx/cy/rx/ry。 */
...
/** ShapeElement：正多边形 / 星，外接圆由 bbox 决定。 */
...
/** BrushPoint：单个笔触采样点，相对 BrushStrokeElement 左上原点。 */
...
/**
 * BrushStrokeElement：用户用笔刷工具画出的连续笔迹。
 *
 * 与 path 元素的关键区别：保留原始采样点 + 压感，不退化为 d 字符串。
 * 渲染时用 Catmull-Rom 拟合 + 变宽（pressureSize）+ 变 alpha（pressureOpacity）。
 */
export interface BrushStrokeElement extends BaseElement {
    type: 'brush';
    /** RDP 简化后的采样点；相对 (element.x, element.y) */
    points: BrushPoint[];
    /** 基础大小（px），[1, 64] */
    size: number;
    /** 笔刷填充：SolidFill / LinearGradient / RadialGradient */
    fill: FillCompat;
...
/**
 * Mask：ImageElement 的可选 SVG path 蒙版。
 * - d：PathDValidator 子集（M/L/Q/C/Z），坐标相对 element bbox (0, 0)..(w, h)；4096 字符长度上限
 * - inverted：false=显示 mask 内（默认），true=显示 mask 外（用 bbox 减去 mask 形状）
 * - featherPx：羽化半径（像素）；null / 缺省 / 0 = 无羽化（硬边），
 *   [1, 32] = 边缘渐变过渡。详见 docs/rendering.md §4.4。
 *
 * RightPanel dropdown 暴露 4 预设（none / circle / roundedRect / ellipse）；
 * lasso 自由路径（Alt + 拖动 image element 在画布上直接画 mask）。
 */
...
/**
 * ImageElement：用户上传的位图。source 是 sha256[:16] 内容寻址 hash；
...
export interface ReadyPayload {
    ...
    protocolVersion: number;
    /**
     * server 同意的 business protocol version；为 undefined 表示对端是
     * 旧后端。client 收到后双向校验 acceptedV === CLIENT_V 才继续。
     */
    accepted_v?: number;
    ...
    // 该 wall 的变量别名映射（fullName → alias）；前端 VariableAliasStore 初始化用。
    aliases?: Record<string, string>;
    // 该 wall 当前所有脚本规则快照（服务端顺序）；前端 ScriptStore 初始化用。
    scripts?: ScriptRule[];
}
...
// ---------- §5.11 变量系统 op payloads ----------
//
// 协议契约见 docs/protocol.md §5.11 + docs/dynamic-data.md §3。
// 后端 op 路由解析以下 payload；ack 形态另见 docs/dynamic-data.md §3.1。
...
/**
 * 触发方式（docs/protocol.md §7 / docs/timeline.md §5）。时间轴内置三种。
 */
...
/**
 * 脚本触发器多态联合（docs/scripting.md §2.2 + scripting-0.7.1.md §6）。
...
export type ScriptAction =
    ...
    // sendMessage 加 target（'trigger' 给触发玩家 / 'all' 全服广播）。后端 Deserializer
...
/**
 * 试跑单步轨迹（S→C 推送 op {@code script.trace} 的 steps 项）。
