...
/** 积木编辑器右侧预览框的折叠态 + 占宽 %（持久化，照 SNAP_KEY 范式）。 */
 ...
 * - {@code 'hand'}：Figma 风格手型工具。任何位置 mousedown+drag = pan 画布；按住 Space 临时切到此工具。
 * - {@code 'line' / 'arrow' / 'circle' / 'star'}：进入"待绘制"状态，cursor = crosshair；
 *   接 canvas mousedown/move/up 实现 drag-to-create。
 */
 ...
 *  paint-bucket 是 click 工具：单击空白处填充 gap、单击元素内部 recolor，
...
/** 是否是笔刷工具（走 PointerEvent + BrushController 而非 drag-to-create）。 */
 ...
 * 仅前端状态，不与 WS 协议交互。
 *
 * <p>选中模型升级为多选。{@link #selectedIds} 是单一真相；{@link #selectedElementId}
 * 仍保留为 computed（size==1 时返该 id），让旧的单选代码可以零修改继续工作。</p>
 */
export const useUiStore = defineStore('ui', () => {
    const themeStore = useThemeStore();
    /**
     * theme 已迁到 {@link useThemeStore}（preset flavor + accent + radius）。
    ...
    /** 变量管理面板（VariablePanel）开关。TopBar 按钮 / ESC 触发。 */
    const variablePanelOpen = ref(false);
    /** 列车时刻表管理 modal 开关。TopBar Calendar 按钮触发。 */
    const scheduleManagerOpen = ref(false);
    /** 铁路网络管理 modal 开关。TopBar Train 按钮 → 二级菜单触发。 */
    const railNetworkOpen = ref(false);
    /** 积木脚本编辑器全屏 overlay 开关。TopBar Puzzle 按钮触发。 */
    const scriptEditorOpen = ref(false);
    /** 所有当前选中的元素 id。size > 1 时为多选。 */
    ...
    /** 当前正在 inline 重命名的图层 id。LayerPanel 双击 layer name 设；保存或 ESC 清。 */
    ...
    // theme DOM 应用已迁到 themeStore（applyFlavor/Accent/Radius）。
    ...
    function toggleTheme() {
        // 委托给 themeStore。Latte ↔ Frappé 二态切；其他 flavor（macchiato）
    ...
    /**
     * wall 切换时重置 wall-scoped 状态；用户偏好（theme / locale / activeTool / zoom /
...
/** 预览框占宽 %。坏值 / 缺失 → 默认 35；越界 clamp 到 [20,70]。 */
