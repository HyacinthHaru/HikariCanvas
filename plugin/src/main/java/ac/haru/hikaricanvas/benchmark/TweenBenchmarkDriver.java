...
import ac.haru.hikaricanvas.script.engine.TweenScheduler;
import ac.haru.hikaricanvas.script.engine.TraceStep;
...
public final class TweenBenchmarkDriver {
    ...
    private static Percentiles measureN(int n, int warmupIters, int measuredIters) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            List<ac.haru.hikaricanvas.script.Action> body = new ArrayList<>();
            for (int t = 0; t < TARGETS_PER_WALL; t++) {
                body.add(new ac.haru.hikaricanvas.script.Action.SetElementProperties(
                        "elem-" + t, Map.of("x", "100", "y", "50", "opacity", "0.8"), null));
            }
            ac.haru.hikaricanvas.script.Action.TweenBlock tb =
                    new ac.haru.hikaricanvas.script.Action.TweenBlock(10_000L, Easing.LINEAR, body);
    ...
    private static ProjectState buildWallState(int elemCount) {
        List<ac.haru.hikaricanvas.state.Element> elements = new ArrayList<>(elemCount);
     ...
     */
    private static final class FakeTickerControl implements ac.haru.hikaricanvas.script.engine.TickerControl {
        volatile long renderCount = 0L;

        @Override
        public ac.haru.hikaricanvas.render.AnimationTicker.Result play(String wallId, String timelineId) {
            return ac.haru.hikaricanvas.render.AnimationTicker.Result.WALL_NOT_FOUND;
        }
        ...
        @Override
        public ac.haru.hikaricanvas.render.AnimationTicker.Result seek(String wallId, String timelineId, long atMs) {
            return ac.haru.hikaricanvas.render.AnimationTicker.Result.WALL_NOT_FOUND;
        }
