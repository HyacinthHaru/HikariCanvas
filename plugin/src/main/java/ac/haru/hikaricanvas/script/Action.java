 ...
 * {@link ac.haru.hikaricanvas.state.KfValue} 同范式（{@link ActionSerializer} /
...
public sealed interface Action permits
     ...
     * {@link ac.haru.hikaricanvas.script.engine.TweenScheduler}。</p>
     *
     ...
     *
     * <p><b>easing</b>：复用 {@link ac.haru.hikaricanvas.state.Easing}（0.6 时间轴现成；
     * {@code null} 表现为 {@link ac.haru.hikaricanvas.state.Easing#LINEAR}，校验层拒 null）。</p>
     *
     * <p>{@code durationMs ∈ [1, 60_000]}（校验常量 {@link ScriptRuleValidator#TWEEN_DURATION_MAX}）。</p>
     */
    record TweenBlock(long durationMs, ac.haru.hikaricanvas.state.Easing easing,
                      java.util.List<Action> body) implements Action {
