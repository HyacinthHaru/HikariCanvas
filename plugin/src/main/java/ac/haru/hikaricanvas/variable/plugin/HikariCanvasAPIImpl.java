...
import ac.haru.hikaricanvas.api.PluginNamespaceException;
...
public final class HikariCanvasAPIImpl implements HikariCanvasAPI {
     ...
     *
     * <p>package-private：只允许 {@link ac.haru.hikaricanvas.HikariCanvas} 主类（同 plugin
    ...
    private void doSetVariable(String namespace, String key, String value,
        ...
        ac.haru.hikaricanvas.variable.VarType type = inferType(namespace, key);
        try {
    ...
    static ac.haru.hikaricanvas.variable.VarType convertVarType(VarType apiType) {
        return switch (apiType) {
            case STRING -> ac.haru.hikaricanvas.variable.VarType.STRING;
            case NUMBER -> ac.haru.hikaricanvas.variable.VarType.NUMBER;
            case BOOLEAN -> ac.haru.hikaricanvas.variable.VarType.BOOLEAN;
            case COLOR -> ac.haru.hikaricanvas.variable.VarType.COLOR;
        };
    }

    /** 查 declared key 类型；未声明 fallback STRING。 */
    private ac.haru.hikaricanvas.variable.VarType inferType(String namespace, String key) {
        PluginNamespaceProvider provider = providers.get(namespace);
        if (provider != null) {
        ...
        return ac.haru.hikaricanvas.variable.VarType.STRING;
    }
