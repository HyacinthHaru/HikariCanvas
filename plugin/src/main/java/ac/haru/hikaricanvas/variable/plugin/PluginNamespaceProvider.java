 ...
 * <h2>用途</h2>
 *
 * <p>让 {@link ac.haru.hikaricanvas.api.HikariCanvasAPI#declareKey} 加入的 keys 通过
 * {@code /api/variable/list-all-namespaces} 端点暴露给前端 Picker。本 Provider 不调度
 * （{@link #refreshInterval()} = {@link Duration#ZERO}），写值走
 * {@link ac.haru.hikaricanvas.api.HikariCanvasAPI#setVariable} 直接调
 * {@link ac.haru.hikaricanvas.variable.VariableStore#setValue}（push 模式 / 事件驱动）。</p>
 *
 * <h2>线程模型</h2>
 *
...
public final class PluginNamespaceProvider implements VariableProvider {
     ...
     * {@link ac.haru.hikaricanvas.api.HikariCanvasAPI#declareKey} 加入。
     */
     ...
     * ac.haru.hikaricanvas.variable.provider.PapiVariableBridge} 的"tracked placeholders"
