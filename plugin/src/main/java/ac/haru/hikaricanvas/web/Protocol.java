 ...
 * （MIN=MAX 同步提升）。<b>本项目取干净切换</b>：前端 bundle 由插件自带分发，客户端与
 * 服务端版本在实际部署中永远匹配，不留双轨窗口。逐版本变更史见 {@code docs/protocol.md}。</p>
 */
public final class Protocol {
    ...
    public static final int SUPPORTED_MIN = 8;

    /** 服务端可接受的最大 business protocol 版本。 */
    public static final int SUPPORTED_MAX = 8;

    /** 协议版本不匹配时关闭 WS 的 close code（与 4001 auth_timeout 同 4xxx 段）。 */
    public static final int CLOSE_PROTOCOL_VERSION_UNSUPPORTED = 4002;

    /**
     * Token 暴力枚举超过限流阈值时关闭 WS 的 close code（配合 {@link TokenRateLimiter}）。
     * 沿用 HTTP 429 语义号段，client 看到 4429 应
     * 显示"请稍后再试"而不是自动重连。
     */
     ...
     * 用 WebSocket 标准的 1008（policy violation）。client 看到 1008 应
