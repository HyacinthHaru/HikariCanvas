...
final class MainThreadPerms {
    ...
    /**
     * 包级静态测试 seam。非 null 时 {@link #resolve} 直接走它（绕开
     * Bukkit / 主线程 hop），让权限拒绝路径可在纯 JVM 单测里
     * dispatch 级驱动（在线被收回节点 → PERMISSION_DENIED + audit 全链）。
     *
     * <p>生产路径 seam 恒 null，仅一次 volatile 读零开销；测试 @AfterEach 必须复位 null。</p>
     */
