...
/**
 * 通用 path 元素（M9 引入）。承载线 / 箭头 / 软线 / 点 五种用途，由 d 串 + marker 决定形态。
 *
 * <p><b>d 字段：</b> SVG path 命令子集 —— {@code M / L / Q / C / Z}（大写绝对，小写相对）。
 * 由 {@link PathDValidator#validate} 词法校验；真正构造 Path2D 在
 * {@code render/CanvasCompositor} 与前端 {@code PreviewRenderer} 内（M9-B/C 实施）。</p>
 *
 ...
 * 从 stroke 派生（M9-B 实装：arrow size ≈ stroke.width × 3；dot radius ≈ stroke.width）。</p>
 *
 * <p>opacity / blendMode / renderMode 与其他元素同语义（M8-A/E 已实装）。</p>
 */
