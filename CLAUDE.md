...
- **P3（20h）** 内置 Provider：13 个系统变量（`server.time` / `wall.alias` / `scoreboard.*` 等）+ PAPI 桥接（`papi/%placeholder%`）+ Manual Schedule provider（兜底列车 ETA）
  - **P3-J（Wave 1）** ✅ **2026-05-19 完成**。基础设施 + SystemProvider + ScoreboardProvider：VariableProvider 接口扩展 `declaredKeys()` / `isDynamic()` + 新 record `DeclaredKey`；VariableStore 加 `registerDynamicLookupHook` + `notifyDynamicLookup`（双形态 namespace 解析）；双端 interpolator 加 `wall.*` 注入（→ `system:<wallId>/wall.X`）+ `scoreboard.*` alias（dot→slash，与 Provider 存储侧约定一致）；SystemVariableProvider（全局 8 + per-wall 4 = 12 变量，独立 TTL 1s~1h，DataSource 接口注入抽象 Bukkit/WallRepo）；ScoreboardVariableProvider（动态 namespace，混合模式：interpolator miss → notifyDynamicLookup → handleDynamic 自动注册 + 10s refresh 切主线程读 scoreboard）；ProviderBootstrap.initialize 签名改 `(store, plugin, wallRepo)`。**1 commit** / **527 backend + 79 frontend test 全绿** / shadow jar 161 MB / 0 baseline 漂移
  - **P3-K（Wave 2）** ✅ **2026-05-19 完成**。PapiVariableBridge：reflection 软依赖（PlaceholderAPI 类不在 classpath 时 noop）+ 动态注册（papi/<encoded> placeholder 形态）。详见 commit `219f731`
  - **P3-L（Wave 2）** ✅ **2026-05-19 完成**。ManualScheduleProvider 全栈（兜底列车 / 公交时刻表）：V012 migration（wall_schedules + schedule_entries 双表 FK CASCADE）+ ScheduleDao CRUD + ManualScheduleProvider（per-wall namespace `schedule:<wallId>` / 4 key `next_departure/destination/eta_minutes/is_arriving` / 30s refresh / 过零点 ETA + 5min is_arriving 阈值）+ ScheduleOpDispatcher 5 WS op + canvas.schedule.{own,any} 权限节点 + 双端 interpolator `schedule.*` 注入 + 前端 ScheduleManagerModal + EntryDialog + Pinia ScheduleStore + TopBar Train 按钮 + i18n 30+ key 中英。**1 commit** / **631 backend + 96 frontend test 全绿**
  - **P3-M（Wave 2）** ✅ **2026-05-19 完成**。`/api/variable/list-all-namespaces` 端点 + VariablePicker metadata 接入：VariableMetadataHandler 聚合 daemon.registeredProviders.declaredKeys + 5s server-side cache + sessionId 鉴权；前端 mergeMetadata 把 declared keys 与 store cached value 合并喂给 picker。详见 commit `c975996`
  - **P3-N（Wave 3 收尾）** VariableMetadataHandler 装配到 WebServer + docs/protocol.md §5.12 schedule op 文档化 待启动
