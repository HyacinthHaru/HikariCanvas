...
- **字体**：只打包 SIL OFL 1.1 协议字体；M22 起内置 **20 枚字体矩阵**：
  ...
  - Gradle `downloadFonts` 构建期抓到 `build/downloaded-fonts/`（SHA-256 pin）→ `syncFontsToWeb` 同步到 `web/public/fonts/` 供前端 `@font-face` 使用 + `processResources` 合并进 shadow jar 供后端 `FontRegistry` 使用 + M20 generateGlyphMetrics 自动按 bundledFonts iterate。仓库不入字体文件，`.gitignore` 排除
