...
- **P3（20h）** 内置 Provider：13 个系统变量（`server.time` / `wall.alias` / `scoreboard.*` 等）+ PAPI 桥接（`papi/%placeholder%`）+ Manual Schedule provider（兜底列车 ETA）
  ...
  - **P3-K（Wave 2）** PapiVariableBridge 待启动
  - **P3-L（Wave 3）** ManualScheduleProvider 待启动
  - **P3-M（Wave 4）** `/api/variable/list-all-namespaces` 端点 + 前端 Picker mount fetch metadata 待启动
