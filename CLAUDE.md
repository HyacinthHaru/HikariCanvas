...
| 0.7.0 | Scratch-like 视觉运行时（**自写积木画布**，Blockly 否决；6 触发器 + 8 动作 + 条件分支；后端唯一执行器 + 真试跑轨迹高亮；命令走服主白名单模板；设计总纲 `docs/scripting.md`，D1-D8 已固化 2026-06-10） | ~340h | **进行中**（P1 ✅ 2026-06-10 数模+V017+协议 v4+前端镜像，后端 1378/前端 529 → P2 执行引擎+MVP 闸（先清 scripting.md §11 P2 账单）→ P3 游戏事件层 → P4 积木引擎 → P5 积木内容 → P6 收尾） |
