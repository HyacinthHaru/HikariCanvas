...
- **字体**：只打包 SIL OFL 1.1 协议字体；M21 起内置 **7 枚字体矩阵**：
  - 中文：`source_han_sans`（黑体）/ `source_han_serif`（宋体）/ `ark_pixel`（12px 像素）
  - 西文：`inter`（无衬线）/ `noto_serif`（衬线）/ `jetbrains_mono`（编程等宽）/ `fira_code`（编程含连字）
  - 中文等宽 gap：`source_han_mono` 仅以 122MB OTC 多语合包形式发布，不适合内置；用户需要走 `plugins/HikariCanvas/fonts/`（M20 用户字体 metrics 自动生效）
  - Gradle `downloadFonts` 构建期抓到 `build/downloaded-fonts/`（SHA-256 pin）→ `syncFontsToWeb` 同步到 `web/public/fonts/` 供前端 `@font-face` 使用 + `processResources` 合并进 shadow jar 供后端 `FontRegistry` 使用。仓库不入字体文件，`.gitignore` 排除。其他字体让用户自己放到 `plugins/HikariCanvas/fonts/`
